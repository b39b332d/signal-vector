...
def draw_vec(file_path):
    ...
    colors_b = ['#6E8FB2', '#CAA66A', '#E5A79A', '#BD9AAE', '#9BB8D6',
            '#D0D08A', '#F5DDB7', '#F5C9BC', '#F3B4BD', '#CEA1B5']
    def plot3d(ax,S_win,v,title,legend=None):
        ...
        ax.scatter(S_win[0], S_win[1], S_win[2],c=colors[1],s=3,label=title[0])
        ...
        ax.quiver(*(k+v*(v@(q0-k))), *v,
                length = v@(q1-q0)*1.0,color=colors[4],arrow_length_ratio=0.1,label=title[1])
        ...
        # ax.text3D(*(k+v*(v@(q1-k))- v*(v@(vec_view))*0.5 + 0.1*np.cross(cam_vec/np.linalg.norm(cam_vec),v*(v@(vec_view)))),"({:.4f},{:.4f},{:.4f})".format(*v),v)        


        # ax.quiver(*(k+v*(v@(q0-k))), *cam_vec,
        #         length = v@(q1-q0)*1.0,color='g',arrow_length_ratio=0.1,label='Fit Vector')
        ax.autoscale(False)
        font_s = fm.FontProperties(family='Times New Roman', size=13)
        ax.set_title("( {:.4f},{:.4f},{:.4f} )".format(*v), fontproperties=font_s)
        # if legend is not None:
        ax.legend(loc='upper left',prop=font_S,frameon=False,labelcolor=[colors_b[1], colors_b[4]],markerscale=2)
