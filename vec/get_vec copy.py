...
sec_idx=0
sec= [ "stationary","talking","slow transition" , "fast transition", "slow rotation", "fast rotation"]
def norm(sig):
...
def get_vec(file_path,draw_i):
    ...
    raw_signal = np.load(rgb_path)*255
    ...
    def plot3d(ax,S_win,v,k,title,legend=None):
        font_S = fm.FontProperties(family='Times New Roman',math_fontfamily='dejavuserif', size=12)
        global sec_idx
        ...
        ax.scatter(S_win[0], S_win[1], S_win[2],s=2,c=colors[1],label=r'${RGB}_{raw}$, '+sec[sec_idx])
        sec_idx+=1
        ...
        ax.set_xlabel("R",fontproperties=font_S)
        ax.set_ylabel("G",fontproperties=font_S)
        ax.set_zlabel("B",fontproperties=font_S)
        ...
        ax.legend(loc='upper left',frameon=False,prop=font_S,markerscale=3)

    if True:

        S_win = S_raw[:,i:i+window_length]
        v,k = ls_line(S_win)
        ax = fig.add_subplot(2, 3, draw_i, projection='3d')
        plot3d(ax,S_win,v,k,"Raw Signal")

        # ax = fig.add_subplot(1, 3, 2, projection='3d')
        # S_win = H[:,i:i+window_length]
        # v,k = ls_line(S_win)
        # plot3d(ax,S_win,"BVP Signal")

        # ax = fig.add_subplot(1, 3, 3, projection='3d')
        # S_win = M[:,i:i+window_length]
        # v,k = ls_line(S_win)
        # plot3d(ax,S_win,"Noise Signal")
...
if __name__ == "__main__":
    ...
    file_s=["01-01","01-02","01-03","01-04","01-05","01-06"]
    i=0
    
    fig = plt.figure(figsize=(9.6, 6.4))
    for f in file_s:
        file_path = f"/tank/数据集/PURE/{f}/{f}.json"
        i+=1
        get_vec(file_path,i)
